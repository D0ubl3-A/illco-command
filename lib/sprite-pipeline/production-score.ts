export const PRODUCTION_SCORE_WEIGHTS = {
  architectureOrchestration: 1200, continuityState: 1000, manifestIntegrity: 1000,
  renderTruthfulness: 1200, duplication: 1000, characterCoverage: 900,
  fxTextureCoverage: 900, visualQuality: 1000, scalabilityOperations: 900,
...
export type ProductionScoreInput = {
  validatedCharacters: number; validatedFx: number;
  expectedCharacters?: number; expectedFx?: number; packageTargets: string[];
  packageVerificationPassed: boolean; exactHashesUnique: boolean;
  perceptualDuplicateScanPassed: boolean; ownershipCoveragePassed: boolean;
  transitionCoveragePassed: boolean; crashRecoveryPassed: boolean;
  sequenceSynchronizationPassed: boolean; originalityReviewPassed: boolean;
  publicationGatePassed: boolean; publicationFailureRate: number;
  mandatoryTestsExecuted: boolean; unresolvedSeverityNineOrTen: number;
  mandatoryTestPassRate: number; blockerTestFailures: number;
  renderTruthfulnessPassed?: boolean; visualQualityPassed?: boolean;
};
export type ProductionScoreResult = {
  rawScore: number; score: number; capped: boolean;
  categories: Record<Category, number>; blockers: string[]; gatePassed: boolean;
};
...
export function calculateProductionScore(input: ProductionScoreInput): ProductionScoreResult {
  ...
  const categories: Record<Category, number> = {
    ...
    renderTruthfulness: renderTruth ? 1200 : 0,
    duplication: input.exactHashesUnique && input.perceptualDuplicateScanPassed ? 1000 : input.exactHashesUnique ? 350 : 0,
    characterCoverage: Math.floor(900 * characterCoverage), fxTextureCoverage: Math.floor(900 * fxCoverage),
    visualQuality: visualQuality ? 1000 : 0, scalabilityOperations: input.crashRecoveryPassed ? 900 : 350,
    commercialEngineReadiness: input.packageVerificationPassed && allTargets ? 900 : input.packageVerificationPassed ? 500 : 0,
  };
